...
def vcheck(current, required):
    from distutils.version import StrictVersion
    return StrictVersion(current) > StrictVersion(required)

try: # pragma: no cover
    import numpy
    assert vcheck(numpy.version.version, '1.3'), "tapir requires >= numpy 1.3"
except ImportError: # pragma: no cover
    raise ImportError('numpy does not seem to be installed. Please see the user guide.') # pragma: no cover

try: # pragma: no cover
    import scipy
    assert vcheck(scipy.version.version, '0.9'), "tapir requires >= scipy 0.9.0"
except ImportError: # pragma: no cover
